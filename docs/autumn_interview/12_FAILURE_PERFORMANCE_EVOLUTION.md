...
Gateway readiness 检测 gRPC 非 Ready，从 Service 摘除；客户端发送失败后复用 client_msg_id 重试。多 Logic 时由服务发现选择健康实例。
...
先测单机基线，再定位瓶颈，再横向扩容。Gateway 看连接/推送队列，Logic 看数据库/Redis，Transfer 看分区和消费速率。实例数不是越多越好，会增加连接和协调成本。
