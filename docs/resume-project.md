...
- 使用 gRPC + Etcd 实现 Logic 服务发现与内部 RPC 调用，Gateway 侧使用 go-zero zRPC `p2c_ewma` 负载均衡选择健康节点，支持本地多 Gateway 实例联调。
...
分布式即时通讯系统，核心开发。基于 Go-Zero 构建 Gateway + Logic + Transfer 分层架构，Gateway 使用 go-zero REST 脚手架承接登录接口与 WebSocket 握手，Logic 使用 go-zero zRPC 脚手架承接内部 gRPC 服务；通过 Etcd 实现 Logic 服务注册发现，并使用 zRPC `p2c_ewma` 在健康节点间负载均衡；使用 Protobuf 二进制协议优化消息传输，使用 Redis Lua 生成会话级 Sequence ID，结合 ACK 未确认消息补偿解决弱网下乱序与漏发；引入 Kafka 异步扩散群聊消息并增加重试/死信处理，降低同步扇出压力；网关层接入 JWT、限流、心跳、Origin 白名单和 uid 固定分片有界 worker pool，提升长连接场景下的稳定性与安全性。
...
- 基于 gRPC + Etcd 实现 Logic 服务注册发现，通过 zRPC `p2c_ewma` 负载均衡调用 Logic；结合 Redis 在线路由映射完成跨 Gateway 精准投递。
...
- 面向游戏运营场景实现活动配置版本化、职责分离审批、灰度发布、Outbox 缓存同步与管理员回滚；使用 RBAC 和统一审计记录管理操作。
- 实现道具批量发放事务，以 `grant_request_id + user_id + item_id` 唯一约束保证幂等，支持重复请求结果查询、批次失败回滚与失败审计。
...
核心工作：
- 基于 gRPC + Etcd 实现服务注册发现，支持本地多 Gateway 实例联调，结合 zRPC `p2c_ewma` 与 Redis 在线状态中心完成跨节点消息路由。
...
核心功能：
- 基于 gRPC + Etcd 实现 Logic 服务注册与发现；Gateway 侧通过 zRPC `p2c_ewma` 选择 Logic 节点，Logic 侧结合在线状态映射与定向 Pub/Sub 完成跨节点单聊路由。
- 基于会话级 `seq`、接收方 ACK 与 pending 结构实现消息排序和失败补偿；Pub/Sub 仅用于在线实时通知，不作为可靠队列，ACK 未返回时保留未确认消息用于重连回放。
- 基于 Kafka + Transfer 服务实现群聊异步扩散，解耦网关连接层与群发链路；在网关侧补充心跳保活、JWT 鉴权、限流与 uid 固定分片有界 worker pool，提升高并发连接场景下的稳定性。
