...
**为什么需要分片工作池？** 有界队列限制并发 gRPC 调用和内存增长；固定 uid 分片让同一发送者始终由同一 shard 串行处理，同时不同 shard 保持并行。服务端对拒绝结果记录指标并返回结构化错误帧，客户端对可重试错误复用原 `client_msg_id`，执行有上限的指数退避加随机抖动，避免过载时立即重试形成同步风暴。
