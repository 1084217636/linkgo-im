...
当前可靠性升级的学习入口仍然是原有章节，不另开第二套教材：第 05 章看 `before_seq/limit` 历史 cursor 和 `acked_seq` 数据语义，第 09、11 章看 Redis 快速补偿与 MySQL 长期兜底，第 12、13 章看群成员初始化、写扩散边界和会话游标，第 21 章执行追加检查。先读代码和测试，再回答章节末的面试追问。
