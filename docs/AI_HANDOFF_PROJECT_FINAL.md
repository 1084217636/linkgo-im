...
7. Gateway 固定分片队列已经完成，但队列拒绝目前仍只记录日志和指标，尚未向客户端返回可识别的 `SERVER_BUSY` 错误帧。
8. Transfer 仍需补 Kafka 手动提交、收件人进度与 lease/retry 语义，证明消费重启和部分投递失败时不会静默丢失。
...
1. 核心 IM 可靠性收口
   ...
   - 队列满、鉴权失败和服务端拒绝必须回传结构化错误帧，不能只写日志。
   - Transfer 使用 Kafka 手动提交，记录收件人进度并补 lease/retry，进程重启后可安全续跑。
