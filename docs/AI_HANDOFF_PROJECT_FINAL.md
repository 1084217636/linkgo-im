...
8. `[已完成 V13.3]` Transfer 已改为 Kafka `FetchMessage + CommitMessages` 手动提交，只有投递、retry 发布或 DLQ 发布成功后才推进 offset；接收方 lease 状态机仍需在 V13.4 完成。
...
1. 核心 IM 可靠性收口
   ...
   - `[待完成 V13.4]` 记录收件人 processing owner/lease/done 状态，进程重启后可安全续跑。
