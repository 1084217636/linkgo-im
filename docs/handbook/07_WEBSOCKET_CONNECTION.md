...
当前代码已经有一个演示级自动重连流程，但它还不是完整的移动端登录恢复方案。

当前网页：

- 提供“重连 WS”按钮。
- 心跳超时会主动关闭连接。
- 非主动关闭时，`onclose` 会调用 `scheduleReconnect()`，按指数退避并加入随机抖动，单次等待最长 30 秒。
- 浏览器恢复网络时，`online` 事件也会安排重连。
- 重连时会重新带上 JWT、当前会话和内存中的 `lastAckedSeq`，让服务端尝试短期补偿。
- 刷新页面后 Token 和会话进度会丢失，需要重新登录，因为当前 Token 和游标只保存在 JavaScript 内存中。

因此面试时可以说客户端已经实现演示级心跳、自动退避重连和短期进度恢复；不能说已经完成 Token 刷新、持久化设备游标、完整多设备同步和移动端级别的离线恢复。
