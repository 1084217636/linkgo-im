...
为什么从 MySQL 找回旧消息后不能跳过验权直接重投？否则发送者被拉黑、移出群或禁言后，仍可以利用旧 ID 触发过期消息。代价是权限撤销后，已落库但未投递的旧消息也会被拒绝恢复。当前消息行没有持久的 `delivery_pending/delivered` 状态，因此选择 fail-closed；完整解法是 Outbox/投递状态由服务端恢复，而不是让客户端任意触发旧消息重投。
