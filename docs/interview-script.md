...
我没有让多个 worker 共同抢一个全局队列，而是按 uid 做固定分片。每个 shard 只有一个 worker 和有界队列，所以同一 uid 的消息按入队顺序提交，不同 shard 又能并行；队列满、池关闭和 context 取消都有独立结果与指标。当前还没有把 queue_full 回写成客户端错误帧，这属于下一步背压协议。
