...
> 本文档用于学完教材后快速定位函数，不负责第一次解释概念。第一次学习请从 [`docs/handbook/README.md`](../handbook/README.md) 开始。
...
```text
当前 Logic 仍同步查询并解析完整收件人列表；Kafka 把消息入库后的“逐收件人投递”移交给 Transfer，使投递消费者可独立扩容并保留 retry / DLQ，但没有消除 Logic 侧随群规模增长的成员解析成本。
```
