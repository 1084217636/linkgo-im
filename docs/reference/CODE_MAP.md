...
> 本文档是学完教材第 19 章后的源码索引，不是入门阅读起点。第一次学习请从 [`docs/handbook/README.md`](../handbook/README.md) 开始；如与代码冲突，以当前代码和主教材为准。

## 1. 服务分层

| 服务 | 目录 | 职责 |
| --- | --- | --- |
| Gateway | `cmd/gateway/` | REST API、JWT 鉴权、WebSocket 建连、心跳、ACK、回放 Redis 未确认短期消息、转发消息到 Logic |
...
| WebSocket | `cmd/gateway/internal/handler/websockethandler.go` | JWT user_id、获取 Logic client、升级 WS、写 Redis route、回放 pending/timeline 短期消息 |
...
需要沿主链路查源码时再按下面顺序定位；第一次学习仍以 handbook 的 `00 → 21` 为唯一顺序。
