// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v3.12.4
// source: api/protocol.proto

package api

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// 定义消息类型枚举
type MsgType int32

const (
	MsgType_NORMAL    MsgType = 0 // 普通消息
	MsgType_HEARTBEAT MsgType = 1 // 心跳包
)

// Enum value maps for MsgType.
var (
	MsgType_name = map[int32]string{
		0: "NORMAL",
		1: "HEARTBEAT",
	}
	MsgType_value = map[string]int32{
		"NORMAL":    0,
		"HEARTBEAT": 1,
	}
)

func (x MsgType) Enum() *MsgType {
	p := new(MsgType)
	*p = x
	return p
}

func (x MsgType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MsgType) Descriptor() protoreflect.EnumDescriptor {
	return file_api_protocol_proto_enumTypes[0].Descriptor()
}

func (MsgType) Type() protoreflect.EnumType {
	return &file_api_protocol_proto_enumTypes[0]
}

func (x MsgType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MsgType.Descriptor instead.
func (MsgType) EnumDescriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{0}
}

type ChatMessage struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	FromUserId string                 `protobuf:"bytes,1,opt,name=from_user_id,json=fromUserId,proto3" json:"from_user_id,omitempty"`
	ToUserId   string                 `protobuf:"bytes,2,opt,name=to_user_id,json=toUserId,proto3" json:"to_user_id,omitempty"` // 如果是群聊，这个字段可以留空，或者忽略
	Content    string                 `protobuf:"bytes,3,opt,name=content,proto3" json:"content,omitempty"`
	Seq        int64                  `protobuf:"varint,4,opt,name=seq,proto3" json:"seq,omitempty"`
	Type       MsgType                `protobuf:"varint,5,opt,name=type,proto3,enum=api.MsgType" json:"type,omitempty"`
	// 【新增】群ID
	GroupId       string `protobuf:"bytes,6,opt,name=group_id,json=groupId,proto3" json:"group_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChatMessage) Reset() {
	*x = ChatMessage{}
	mi := &file_api_protocol_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChatMessage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChatMessage) ProtoMessage() {}

func (x *ChatMessage) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChatMessage.ProtoReflect.Descriptor instead.
func (*ChatMessage) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{0}
}

func (x *ChatMessage) GetFromUserId() string {
	if x != nil {
		return x.FromUserId
	}
	return ""
}

func (x *ChatMessage) GetToUserId() string {
	if x != nil {
		return x.ToUserId
	}
	return ""
}

func (x *ChatMessage) GetContent() string {
	if x != nil {
		return x.Content
	}
	return ""
}

func (x *ChatMessage) GetSeq() int64 {
	if x != nil {
		return x.Seq
	}
	return 0
}

func (x *ChatMessage) GetType() MsgType {
	if x != nil {
		return x.Type
	}
	return MsgType_NORMAL
}

func (x *ChatMessage) GetGroupId() string {
	if x != nil {
		return x.GroupId
	}
	return ""
}

// --- Logic 服务接口 (Gateway -> Logic) ---
type PushMsgReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"` // 谁发的
	Content       []byte                 `protobuf:"bytes,2,opt,name=content,proto3" json:"content,omitempty"`             // 原始内容
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PushMsgReq) Reset() {
	*x = PushMsgReq{}
	mi := &file_api_protocol_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PushMsgReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PushMsgReq) ProtoMessage() {}

func (x *PushMsgReq) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PushMsgReq.ProtoReflect.Descriptor instead.
func (*PushMsgReq) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{1}
}

func (x *PushMsgReq) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *PushMsgReq) GetContent() []byte {
	if x != nil {
		return x.Content
	}
	return nil
}

type PushMsgReply struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PushMsgReply) Reset() {
	*x = PushMsgReply{}
	mi := &file_api_protocol_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PushMsgReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PushMsgReply) ProtoMessage() {}

func (x *PushMsgReply) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PushMsgReply.ProtoReflect.Descriptor instead.
func (*PushMsgReply) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{2}
}

type UserLoginReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        string                 `protobuf:"bytes,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserLoginReq) Reset() {
	*x = UserLoginReq{}
	mi := &file_api_protocol_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserLoginReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserLoginReq) ProtoMessage() {}

func (x *UserLoginReq) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserLoginReq.ProtoReflect.Descriptor instead.
func (*UserLoginReq) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{3}
}

func (x *UserLoginReq) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

type UserLoginReply struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UserLoginReply) Reset() {
	*x = UserLoginReply{}
	mi := &file_api_protocol_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UserLoginReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UserLoginReply) ProtoMessage() {}

func (x *UserLoginReply) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UserLoginReply.ProtoReflect.Descriptor instead.
func (*UserLoginReply) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{4}
}

// --- 【新增】Gateway 服务接口 (Logic -> Gateway) ---
// Logic 告诉 Gateway：把这条消息推给某个人
type SendToUserReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	TargetUserId  string                 `protobuf:"bytes,1,opt,name=target_user_id,json=targetUserId,proto3" json:"target_user_id,omitempty"` // 发给谁
	Content       []byte                 `protobuf:"bytes,2,opt,name=content,proto3" json:"content,omitempty"`                                 // 发什么
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendToUserReq) Reset() {
	*x = SendToUserReq{}
	mi := &file_api_protocol_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendToUserReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendToUserReq) ProtoMessage() {}

func (x *SendToUserReq) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendToUserReq.ProtoReflect.Descriptor instead.
func (*SendToUserReq) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{5}
}

func (x *SendToUserReq) GetTargetUserId() string {
	if x != nil {
		return x.TargetUserId
	}
	return ""
}

func (x *SendToUserReq) GetContent() []byte {
	if x != nil {
		return x.Content
	}
	return nil
}

type SendToUserReply struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Success       bool                   `protobuf:"varint,1,opt,name=success,proto3" json:"success,omitempty"` // 必须有这一行
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendToUserReply) Reset() {
	*x = SendToUserReply{}
	mi := &file_api_protocol_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendToUserReply) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendToUserReply) ProtoMessage() {}

func (x *SendToUserReply) ProtoReflect() protoreflect.Message {
	mi := &file_api_protocol_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendToUserReply.ProtoReflect.Descriptor instead.
func (*SendToUserReply) Descriptor() ([]byte, []int) {
	return file_api_protocol_proto_rawDescGZIP(), []int{6}
}

func (x *SendToUserReply) GetSuccess() bool {
	if x != nil {
		return x.Success
	}
	return false
}

var File_api_protocol_proto protoreflect.FileDescriptor

const file_api_protocol_proto_rawDesc = "" +
	"\n" +
	"\x12api/protocol.proto\x12\x03api\"\xb6\x01\n" +
	"\vChatMessage\x12 \n" +
	"\ffrom_user_id\x18\x01 \x01(\tR\n" +
	"fromUserId\x12\x1c\n" +
	"\n" +
	"to_user_id\x18\x02 \x01(\tR\btoUserId\x12\x18\n" +
	"\acontent\x18\x03 \x01(\tR\acontent\x12\x10\n" +
	"\x03seq\x18\x04 \x01(\x03R\x03seq\x12 \n" +
	"\x04type\x18\x05 \x01(\x0e2\f.api.MsgTypeR\x04type\x12\x19\n" +
	"\bgroup_id\x18\x06 \x01(\tR\agroupId\"?\n" +
	"\n" +
	"PushMsgReq\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\tR\x06userId\x12\x18\n" +
	"\acontent\x18\x02 \x01(\fR\acontent\"\x0e\n" +
	"\fPushMsgReply\"'\n" +
	"\fUserLoginReq\x12\x17\n" +
	"\auser_id\x18\x01 \x01(\tR\x06userId\"\x10\n" +
	"\x0eUserLoginReply\"O\n" +
	"\rSendToUserReq\x12$\n" +
	"\x0etarget_user_id\x18\x01 \x01(\tR\ftargetUserId\x12\x18\n" +
	"\acontent\x18\x02 \x01(\fR\acontent\"+\n" +
	"\x0fSendToUserReply\x12\x18\n" +
	"\asuccess\x18\x01 \x01(\bR\asuccess*$\n" +
	"\aMsgType\x12\n" +
	"\n" +
	"\x06NORMAL\x10\x00\x12\r\n" +
	"\tHEARTBEAT\x10\x012o\n" +
	"\x05Logic\x121\n" +
	"\vPushMessage\x12\x0f.api.PushMsgReq\x1a\x11.api.PushMsgReply\x123\n" +
	"\tUserLogin\x12\x11.api.UserLoginReq\x1a\x13.api.UserLoginReply2A\n" +
	"\aGateway\x126\n" +
	"\n" +
	"PushToUser\x12\x12.api.SendToUserReq\x1a\x14.api.SendToUserReplyB\aZ\x05./apib\x06proto3"

var (
	file_api_protocol_proto_rawDescOnce sync.Once
	file_api_protocol_proto_rawDescData []byte
)

func file_api_protocol_proto_rawDescGZIP() []byte {
	file_api_protocol_proto_rawDescOnce.Do(func() {
		file_api_protocol_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_api_protocol_proto_rawDesc), len(file_api_protocol_proto_rawDesc)))
	})
	return file_api_protocol_proto_rawDescData
}

var file_api_protocol_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_api_protocol_proto_msgTypes = make([]protoimpl.MessageInfo, 7)
var file_api_protocol_proto_goTypes = []any{
	(MsgType)(0),            // 0: api.MsgType
	(*ChatMessage)(nil),     // 1: api.ChatMessage
	(*PushMsgReq)(nil),      // 2: api.PushMsgReq
	(*PushMsgReply)(nil),    // 3: api.PushMsgReply
	(*UserLoginReq)(nil),    // 4: api.UserLoginReq
	(*UserLoginReply)(nil),  // 5: api.UserLoginReply
	(*SendToUserReq)(nil),   // 6: api.SendToUserReq
	(*SendToUserReply)(nil), // 7: api.SendToUserReply
}
var file_api_protocol_proto_depIdxs = []int32{
	0, // 0: api.ChatMessage.type:type_name -> api.MsgType
	2, // 1: api.Logic.PushMessage:input_type -> api.PushMsgReq
	4, // 2: api.Logic.UserLogin:input_type -> api.UserLoginReq
	6, // 3: api.Gateway.PushToUser:input_type -> api.SendToUserReq
	3, // 4: api.Logic.PushMessage:output_type -> api.PushMsgReply
	5, // 5: api.Logic.UserLogin:output_type -> api.UserLoginReply
	7, // 6: api.Gateway.PushToUser:output_type -> api.SendToUserReply
	4, // [4:7] is the sub-list for method output_type
	1, // [1:4] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_api_protocol_proto_init() }
func file_api_protocol_proto_init() {
	if File_api_protocol_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_api_protocol_proto_rawDesc), len(file_api_protocol_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   7,
			NumExtensions: 0,
			NumServices:   2,
		},
		GoTypes:           file_api_protocol_proto_goTypes,
		DependencyIndexes: file_api_protocol_proto_depIdxs,
		EnumInfos:         file_api_protocol_proto_enumTypes,
		MessageInfos:      file_api_protocol_proto_msgTypes,
	}.Build()
	File_api_protocol_proto = out.File
	file_api_protocol_proto_goTypes = nil
	file_api_protocol_proto_depIdxs = nil
}
