...
LOGIC_BASE="${LOGIC_BASE:-http://127.0.0.1:9002}"
...
check_metrics "transfer metrics" "$TRANSFER_BASE/metrics" "linkgo_"

check_http "logic healthz" "$LOGIC_BASE/healthz"
check_http "logic readyz" "$LOGIC_BASE/readyz"
check_metrics "logic metrics" "$LOGIC_BASE/metrics" "linkgo_"
...
logic_metric_sample="$(curl -fsS --max-time 5 "$LOGIC_BASE/metrics" 2>/dev/null | grep -E '^(# HELP linkgo_|# TYPE linkgo_|linkgo_)' | head -n 40 || true)"

{
  ...
  echo '```'
  echo
  echo "## Logic Metric Sample"
  echo
  echo '```text'
  echo "$logic_metric_sample"
  echo '```'
} >"$REPORT"
